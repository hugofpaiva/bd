...
BEGIN
        SET NOCOUNT ON
            IF EXISTS(SELECT numero FROM Perfumaria.perf.compra_online WHERE numero=@numero)
                BEGIN
                    SELECT pagamento AS TipoPagamento, contribuinte, rating, rastreamento, presente, observacao, telemovel, codigo_postal AS codigoPostal, pais, endereco, apartamento, localidade 
            ...
            ELSE IF EXISTS(SELECT numero FROM Perfumaria.perf.compra_presencial WHERE numero=@numero)
                BEGIN
                    SELECT pagamento AS TipoPagamento, compra.contribuinte, fname as Funcionario
